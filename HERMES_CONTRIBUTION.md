...
email completion, specific rule fixes such as `required_actions`, zero
baseline-win regressions, and no new critical failures before preference rows
are treated as improvement signal.
`release_check.sh` exercises this path by building paired email-completion
...
5. Run a full scenario directory with `flightrecorder run-suite --evidence-handoff`
   ...
   do, including when to dispatch `repair_queue.json` as concrete failed-rule
   work items with evidence refs, bounded snippets, artifacts, and replay
   commands.
...
9. Compare whole baseline/candidate run directories with
   ...
   same-named paired scenarios used the same scenario contract; strict fixture
   replay can opt into source-trace matching with `--contract-scope
   scenario-and-trace`.
