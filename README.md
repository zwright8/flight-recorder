...
The repository includes three completed, public, evidence-bound LoRA results.
GitHub holds the replayable manifests, evaluations, decisions, and checksums.
Where publication review permits it, Hugging Face holds the corresponding
datasets and loadable PEFT adapters tied to pinned base-model revisions rather
than merged full-model checkpoints. Sensitive task content and weights remain
private when the evidence capsule's claim does not require publishing them.
...
| Qwen3-0.6B ARC strategy-router LoRA | 1,561 validated Flight Recorder trajectories; 1,457 visible-family training rows; local MLX and Kaggle-native PEFT replays | Local MLX: base 2/52, either LoRA 50/52, bounded ensemble 52/52. Controlled Kaggle-native router-only replay: base 0/52, step-900 LoRA 17/52 (+32.69 points, zero execution errors). Kaggle submission 55427691 completed, but Kaggle published no score | [Visible evidence](examples/case_studies/arcagi_strategy_student/results/visible_arcagi_a_v1/README.md) · [Kaggle route evidence](examples/case_studies/arcagi_strategy_student/results/kaggle_arcagi2_router_only_v1/README.md) · [Kaggle metrics](examples/case_studies/arcagi_strategy_student/results/kaggle_arcagi2_router_only_v1/metrics.json) |

### ARC result boundary

The ARC result is visible-family teacher imitation, not an unseen or sealed ARC
benchmark score, because transformations from the same 48 source task families
...
delta is verified. LoRA submission 55427691 completed, but Kaggle returned blank
public and private score fields; the base artifact has not received an official
score. No sealed improvement is claimed without scored results for both arms.
