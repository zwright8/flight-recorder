...
- External-eval strict validation now warns on absolute heldout-manifest and
  ...
  governance evidence.
- Heldout-manifest validation replays source suite summaries and scenario bytes
  from the manifest location, rejects stale or incomplete fingerprints and
  duplicate arm sources, while strict validation warns on absolute source refs.
- External-eval planning revalidates held-out semantics instead of trusting a
  copied readiness flag, so forged or stale manifests cannot unlock adapters.
...
- Promotion archives and trainer archives reject absolute, parent-traversing,
  symlinked, or symlink-parent artifact paths before trusting recorded file
  hashes, tree hashes, or self-contained handoff claims. Force replacement is
  limited to valid command-owned archives at filesystem-safe targets.
